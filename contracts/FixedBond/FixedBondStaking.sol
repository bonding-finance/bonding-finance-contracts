...
/**
 * @title Fixed-duration bond staking contract
 * @author Bonding Finance
 */
contract FixedBondStaking is IFixedBondStaking, ReentrancyGuard {
    ...
    constructor(address _vault, address _lpToken) {
        ...
        factory = IFixedBondVault(_vault).factory();
        vault = _vault;
        yToken = IFixedBondVault(_vault).yToken();
        // Note: If _lpToken is 0, the protocol will earn all surplus yield
        lpToken = _lpToken;
        rewardToken = IFixedBondVault(_vault).token();
    }
    ...
    function harvest() public override {
        if (IFixedBondVault(vault).staking() == address(this)) {
            IFixedBondVault(vault).harvest();
        }
